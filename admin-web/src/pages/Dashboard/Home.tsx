...
export default function Home() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      const [ordersResult, activeUsersResult, paymentsResult, shippingResult] =
        await Promise.allSettled([
          isAdmin
            ? commerceService.getAdminOrders()
            : commerceService.getSellerOrders(),
          isAdmin
            ? userService.listManagedUsers("", "active")
            : Promise.resolve([]),
  ...
  const metrics = useMemo(() => {
    ...
    return {
      ...
      revenue,
      pendingOrders: state.orders.filter((order) => isOpenOrder(order.status))
        .length,
      activeUsers: state.activeUsers.length,
      activeSellers: state.activeUsers.filter(
        (account) => roleOf(account.role) === "seller",
      ).length,
      activeCustomers: state.activeUsers.filter(
        (account) => roleOf(account.role) === "customer",
      ).length,
    };
  ...
  return (
    <>
      <PageMeta
        title="Dashboard | Mega Mall Admin"
        description="Mega Mall admin dashboard"
      />
      ...
      <div className="space-y-6">
        ...
        {state.error ? (
          <div className="rounded-lg border border-warning-200 bg-warning-50 px-4 py-3 text-sm text-warning-700 dark:border-warning-500/20 dark:bg-warning-500/10 dark:text-orange-300 break-all">
            {state.error}
          </div>
        ) : null}
        ...
        <div className="grid grid-cols-1 gap-4 lg:grid-cols-3">
          ...
          />
          <MetricCard
            label="Thanh toán đang bật"
            value={formatNumber(
              state.paymentMethods.filter((method) => method.enabled).length,
            )}
            compact
          />
        </div>
        ...
        <div className="grid grid-cols-1 gap-6 xl:grid-cols-3">
          <Panel className="xl:col-span-2">
            ...
            {state.loading ? (
              ...
              <EmptyState>
                Chưa có đơn hàng phù hợp với trạng thái hiện tại.
              </EmptyState>
            )}
          </Panel>

          <Panel>
            <PanelHeader title="Tài khoản active" />
            <div className="space-y-4 p-5">
              {isAdmin ? (
                ...
                <p className="text-sm text-gray-500 dark:text-gray-400">
                  Seller không được xem danh sách tài khoản. Chỉ admin mới có
                  thể xem danh sách tài khoản active.
                </p>
              )}
            </div>
          </Panel>
