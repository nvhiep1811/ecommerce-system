...
-- =============================================================
-- SPRING BOOT SERVICE-BASED E-COMMERCE (SHARED DATABASE)
-- HARDENED V2 FOR IMPLEMENTATION
-- Target: Spring Boot services + PostgreSQL + Mobile app (Expo)
--
-- Principles:
--   * Shared database, but clear table ownership by service/domain
--   * Keep business workflows in Spring services, not heavy DB triggers
--   * Stronger relational integrity before implementation
--   * Hybrid cart model: guest cart on device, authenticated cart on server
-- =============================================================
...
-- Tokenized only. Never store raw card number / CVV.
...
-- Composite uniqueness to support composite FKs from cart/order/inventory
...
-- =============================================================
-- product_id is always present.
-- variant_id is nullable:
--   * NULL   => inventory row for a simple product
--   * non-NULL => inventory row for a specific product variant
...
--   * Guest cart stays on the device (Expo SQLite / local store).
--   * Server DB stores only authenticated-user carts.
--   * On login, backend merges device cart lines into the active server cart.
...
CREATE TABLE IF NOT EXISTS orders (
  ...
  order_status varchar(30) NOT NULL DEFAULT 'pending'
    CHECK (order_status IN ('pending', 'confirmed', 'processing', 'shipping', 'delivered', 'cancelled', 'returned')),
  ...
  fulfillment_status varchar(30) NOT NULL DEFAULT 'pending'
    CHECK (fulfillment_status IN ('pending', 'packed', 'shipping', 'delivered', 'returned', 'cancelled')),
  ...
  payment_method_code varchar(30) NOT NULL
    CHECK (payment_method_code IN ('COD', 'CARD', 'BANK_TRANSFER', 'VNPAY', 'MOMO', 'PAYPAL')),
...
-- Snapshot product info at purchase time
...
CREATE TABLE IF NOT EXISTS shipments (
  ...
  shipment_status varchar(30) NOT NULL DEFAULT 'pending'
    CHECK (shipment_status IN ('pending', 'ready_to_ship', 'shipped', 'in_transit', 'delivered', 'failed', 'returned', 'cancelled')),
  note text,
...
CREATE TABLE IF NOT EXISTS payments (
  ...
  provider varchar(50) NOT NULL
    CHECK (provider IN ('COD', 'CARD', 'BANK_TRANSFER', 'VNPAY', 'MOMO', 'PAYPAL')),
  method varchar(30) NOT NULL
    CHECK (method IN ('COD', 'CARD', 'BANK_TRANSFER', 'VNPAY', 'MOMO', 'PAYPAL')),
...
CREATE TABLE IF NOT EXISTS payment_transactions (
  ...
  transaction_type varchar(30) NOT NULL
    CHECK (transaction_type IN ('authorize', 'capture', 'refund', 'void', 'webhook_update')),
...
-- =============================================================
-- 12. IMPLEMENTATION NOTES FOR SPRING BOOT
-- =============================================================
-- 1) Use @Version on tables that have `version` for optimistic locking:
...
--
-- 2) Order placement should be orchestrated in Spring services:
--      validate cart -> price recheck -> reserve inventory -> create order
--      -> create payment attempt -> handle callback/webhook -> confirm order
--
-- 3) Inventory should be changed through Inventory Service only.
--    Other services should not update stock columns directly.
--
-- 4) Products should be soft-deleted (deleted_at) instead of hard-deleted
--    after they have been referenced in orders.
--
-- 5) Guest cart should stay local on mobile (Expo SQLite).
--    On login, call a merge endpoint to sync into the active server cart.
--
-- 6) Keep the DB strict on integrity, but keep workflows/business rules
--    in Java services, not in heavy triggers.
-- =============================================================
