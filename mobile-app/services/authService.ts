...
class AuthService {
  ...
  async getUserProfile(userId?: string): Promise<{ profile: UserProfile | null; error: string | null }> {
    try {
      ...
      return { profile: null, error: error instanceof Error ? error.message : 'Unable to load profile' };
    }
  }

  async updateProfile(updates: Partial<UserProfile>) {
    try {
      const payload = await apiClient.put<AuthApiResponse['user']>('/users/me', {
      ...
      return { data: null, error: error instanceof Error ? error.message : 'Unable to update profile' };
    }
  }

  async resetPassword(email: string) {
    return { error: 'Password reset flow is not implemented yet', message: null };
  }

  async signInWithGoogle() {
    return { data: null, error: 'OAuth login is not implemented in the new backend yet' };
  }
